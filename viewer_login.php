...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'set_profile') {
        ...
        if (empty($login)) {
            ...
            $error = "Veuillez renseigner votre direction.";
            $step  = 'profile'; $preLogin = $login;
        } elseif (empty($service)) {
            $error = "Veuillez renseigner votre service ou agence.";
            $step  = 'profile'; $preLogin = $login;
        } else {
            ...
            if ($existing) {
                $pdo->prepare("UPDATE viewers SET fullname=?, direction=?, service=?, last_seen=NOW()
                               WHERE windows_login=?")
                    ->execute([$fullname, $direction, $service, $login]);
                $vid = $existing['id'];
            } else {
                $pdo->prepare("INSERT INTO viewers (windows_login, fullname, direction, service)
                               VALUES (?,?,?,?)")
                    ->execute([$login, $fullname, $direction, $service]);
...
input[type=text]{width:100%;padding:11px 14px 11px 38px;background:#0f1117;
  border:1px solid #2d2f31;border-radius:8px;color:#f3f4f6;font-size:.9rem;
  outline:none;transition:border-color .2s;}
input[type=text]:focus{border-color:#1a2e6e;box-shadow:0 0 0 3px rgba(26,46,110,.15);}
  ...
  <div class="login-card">
    ...
    <?php else: ?>
      ...
      <form method="post">
        ...
        <div class="form-group">
          <label for="service">Service / Agence <span style="color:#e8192c">*</span></label>
          <div class="input-wrap">
